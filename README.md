...
## datacloud:compute:deploy – authorization not found (record_not_found)

If you see an error similar to:

```
Retrieving Data Cloud authorization... !
Error: record_not_found: Not found.
```

  It usually means the Data Cloud authorization with the developer name you passed to `--authorization` does not exist for the selected AppLink add-on on the app.

- Verify the AppLink add-on installed on the app:
  - `heroku addons --app <your-app>`
  - If multiple `heroku-applink` add-ons are installed, include `--addon <add-on-name>` in commands below.

- List existing authorizations to confirm the developer name:
  - `heroku applink:authorizations --app <your-app>`
  - If multiple AppLink add-ons exist, run: `heroku applink:authorizations --app <your-app> --addon <add-on-name>`

- If it doesn't exist, create the authorization and complete the browser flow:
  - `heroku datacloud:authorizations:add <developer_name> --app <your-app>`
  - Optionally include a login URL: `--login-url https://login.salesforce.com`

- Retry your deploy:
  - `heroku datacloud:compute:deploy --app <your-app> --authorization <developer_name> --name <package_name> --path <dir> --runtime <runtime>`
  - If multiple AppLink add-ons exist, also include `--addon <add-on-name>`.

Tip: For environment variables, pass `--env` repeatedly as `KEY=VALUE` pairs, e.g. `--env FOO=bar --env BAZ=qux`.

## datacloud:compute:deploy – debugging and payload structure

Use these environment variables during development to aid troubleshooting. These notes live outside the autogenerated Commands section, so they persist across builds.

- DEBUG: when set to any non-empty value, enables additional info logs for `datacloud:compute:deploy` and prints copy-pasteable curl snippets for the POST and PUT steps. It also saves a copy of the deployment zip on disk.
- TMPDIR: optional. If set, the local copy of the zip is saved to `${TMPDIR}/deployment.zip`; otherwise it is saved to `/tmp/deployment.zip`.
- DATA_CLOUD_INSTANCE_URL: optional. Overrides the Salesforce instance URL returned by the authorization for the duration of the deploy.
- DATA_CLOUD_ACCESS_TOKEN: optional. Overrides the access token returned by the authorization for the duration of the deploy.

Default path behavior and required structure when using payload

- If you omit `--path`, the command defaults to zipping the `payload/` directory.
- If you pass `--path payload` (or any path ending in `/payload`), the zip will include a top-level `payload/` folder with the required files:
  - `payload/config.json`
  - `payload/entrypoint.py`
- The command fails if the `payload/` directory or either required file is missing.
...
* [`heroku datacloud:compute:deploy`](#heroku-datacloudcomputedeploy)
...
## `heroku datacloud:compute:deploy`

Zip and deploy Data Cloud Custom Code directly via Salesforce SSOT APIs using stored AppLink authorization

```
USAGE
  $ heroku datacloud:compute:deploy -a <value> --authorization <value> --name <value> [--addon <value>] [-r <value>] [--path
    <value>] [--description <value>] [--env <value>...] [--timeout <value>] [--memory <value>] [--override-token
    <value>]
...
FLAGS
  ...
  -r, --remote=<value>          git remote of app to use
      --addon=<value>           unique name or ID of an AppLink add-on
      --authorization=<value>   (required) AppLink Data Cloud authorization developer name to use
      ...
      --override-token=<value>  Override the access token for testing
      --path=<value>            [default: payload] Path to source directory to zip and deploy. Defaults to 'payload'
                                (expects payload/config.json and payload/entrypoint.py)
...
_See code: [src/commands/datacloud/compute/deploy.ts](https://github.com/heroku/heroku-cli-plugin-applink/blob/v1.0.1/src/commands/datacloud/compute/deploy.ts)_
