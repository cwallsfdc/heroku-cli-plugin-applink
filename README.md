...
## datacloud:deploy – authorization not found (record_not_found)
...
It usually means the Data Cloud authorization with the developer name you passed to `--authorization-name` does not exist for the selected AppLink add-on on the app.
...
- Retry your deploy:
  - `heroku datacloud:deploy --app <your-app> --authorization-name <developer_name> --name <package_name> --path <dir> --runtime <runtime>`
...
## datacloud:deploy – debugging and payload structure
...
- DEBUG: when set to any non-empty value, enables additional info logs for `datacloud:deploy` and prints copy-pasteable curl snippets for the POST and PUT steps. It also saves a copy of the deployment zip on disk.
...
```
USAGE
  $ heroku datacloud:deploy -a <value> --authorization-name <value> --name <value> [--addon <value>] [-r <value>]
    [--path <value>] [--description <value>] [--env <value>...] [--timeout <value>] [--memory <value>] [--override-token
    <value>]
